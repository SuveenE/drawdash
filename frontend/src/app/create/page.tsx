...
import { ImageSidebarSimple } from '@/components/canvas/image-sidebar-simple';
...
export default function CreatePage() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const data = await generateImage({
        ...
        type: requestType,
        save_data: false, // Don't save to database for create page
      });
  ...
  }, [mode, agentTranscript, askPrompt, exportCanvasImage, frameId]);
    ...
    <div className="flex h-full w-full bg-white">
      ...
      <div className="flex flex-1 flex-col bg-white">
        <div className="flex items-center justify-between border-b border-gray-200 bg-white p-3 pl-4 text-left text-sm font-medium text-gray-900">
          <div>
            <span className="text-gray-900">Try DrawDash</span>
          </div>
        </div>
